...
export default function LiquidEther({
    ...
    resolution = 0.5,
    isBounce = false,
    colors = ['#801a24', '#d4a373', '#1a0508'], // Mapped to Brand Colors
    style = {},
    className = '',
    autoDemo = true,
    autoSpeed = 0.6,
    ...
    useEffect(() => {
        ...
        class CommonClass {
            constructor() {
                ...
                this.time = 0; this.delta = 0; this.container = null; this.renderer = null;
                this.lastTime = performance.now(); // High precision timestamp replacing clock
            }
            init(container) {
            ...
            update() {
                ...
                this.delta = (now - this.lastTime) / 1000; // Calculated exact step interval delta
        ...
        const pressure_frag = `precision highp float; uniform sampler2D pressure; uniform sampler2D velocity; uniform vec2 px; uniform float dt; varying vec2 uv; void main(){ float p0 = texture2D(pressure, uv + vec2(px.x, 0.0)).r; float p1 = texture2D(pressure, uv - vec2(px.x, 0.0)).r; float p2 = texture2D(pressure, uv + vec2(0.0, px.y)).r; float p3 = texture2D(pressure, uv - vec2(0.0, px.y)).r; vec2 gradP = vec2(p0 - p1, p2 - p3) * 0.5; gl_FragColor = vec4(texture2D(velocity, uv).xy - gradP * dt, 0.0, 1.0); }`;
        ...
        class Advection extends ShaderPass {
            ...
            init() {
                super.init(); const boundaryG = new THREE.BufferGeometry();
        ...
        class ExternalForce extends ShaderPass {
            ...
            init(simProps) {
                super.init(); const mouseM = new THREE.RawShaderMaterial({ vertexShader: mouse_vert, fragmentShader: externalForce_frag, blending: THREE.AdditiveBlending, depthWrite: false, uniforms: { px: { value: simProps.cellScale }, force: { value: new THREE.Vector2() }, center: { value: new THREE.Vector2() }, scale: { value: new THREE.Vector2(simProps.cursor_size, simProps.cursor_size) } } });
                this.mouse = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), mouseM); this.scene.add(this.mouse);
            }
            update(props) {
                const uniforms = this.mouse.material.uniforms;
                uniforms.force.value.set((Mouse.diff.x / 2) * props.mouse_force, (Mouse.diff.y / 2) * props.mouse_force);
                uniforms.center.value.set(Math.min(Math.max(Mouse.coords.x, -1 + (props.cursor_size * props.cellScale.x)), 1 - (props.cursor_size * props.cellScale.x)), Math.min(Math.max(Mouse.coords.y, -1 + (props.cursor_size * props.cellScale.y)), 1 - (props.cursor_size * props.cellScale.y)));
                super.update();
            }
        }
        ...
        class Viscous extends ShaderPass {
            ...
            update({ viscous, iterations, dt }) {
                ...
                for (let i = 0; i < iterations; i++) {
                    fbo_in = i % 2 === 0 ? this.props.output0 : this.props.output1; fbo_out = i % 2 === 0 ? this.props.output1 : this.props.output0;
        ...
        class Poisson extends ShaderPass {
            ...
            update({ iterations }) {
                let p_in, p_out;
                for (let i = 0; i < iterations; i++) {
                    p_in = i % 2 === 0 ? this.props.output0 : this.props.output1; p_out = i % 2 === 0 ? this.props.output1 : this.props.output0;
        ...
        class Simulation {
            ...
            calcSize() { const w = Math.max(1, Math.round(this.options.resolution * Common.width)), h = Math.max(1, Math.round(this.options.resolution * Common.height)); this.cellScale.set(1.0 / w, 1.0 / h); this.fboSize.set(w, h); }
            ...
            update() {
                ...
                let vel = this.options.isViscous ? this.viscous.update({ viscous: this.options.viscous, iterations: this.options.iterations_viscous, dt: this.options.dt }) : this.fbos.vel_1;
        ...
        class WebGLManager {
            constructor(props) {
                ...
                this.init(); this._loop = this.loop.bind(this); this._resize = this.resize.bind(this); window.addEventListener('resize', this._resize);
            ...
            loop() { if (!this.running) return; this.autoDriver?.update(); Mouse.update(); Common.update(); this.output.update(); rafRef.current = requestAnimationFrame(this._loop); }
            ...
            dispose() { window.removeEventListener('resize', this._resize); document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose(); Common.renderer?.dispose(); }
        }
        ...
        applyOptions(); webgl.start();

        const io = new IntersectionObserver(entries => { isVisibleRef.current = entries[0].isIntersecting; if (isVisibleRef.current && !document.hidden) webglRef.current?.start(); else webglRef.current?.pause(); }, { threshold: [0, 0.01] });
