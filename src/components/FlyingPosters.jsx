...
// Custom Vertex Shader: Left/Right positioning offset dynamically added to standard flow
const vertexShader = `
...
void main() {
  ...
  // Left/Right staggered dynamic screen displacement base
  float sideDirection = mod(uIndex, 2.0) == 0.0 ? -1.5 : 1.5;
  ...
  float waveEffect = sin(position.x * 2.0 + uTime) * (uSpeed * 0.15);
  newpos.z += waveEffect;
  
  // Smoothly blend side fly-in entry back to baseline center stream
  float entryTransition = smoothstep(5.0, 10.0, uPosition);
  newpos.x += sideDirection * (1.0 - entryTransition) * 2.5;
...
class Canvas {
    constructor({ container, canvas, items, planeWidth, planeHeight, distortion, scrollEase, cameraFov, cameraZ, onScrollUpdate }) {
    ...
    update() {
        ...
        if (this.onScrollUpdate && Math.abs(this.scroll.current - this.scroll.last) > 0.001) {
            this.onScrollUpdate(this.scroll.current);
        }
...
export default function FlyingPosters({
    ...
    // Dynamic Font Management Lists
    const fonts = ['sans-serif', 'serif', 'monospace', 'cursive', 'system-ui', '"Courier New"', 'Impact', 'Georgia'];
    const fontWeights = ['100', '300', '400', '700', '900'];
    ...
    const handleScrollUpdate = (currentScroll) => {
        // Generate pseudorandom styles strictly driven by scroll position scale increments
        const seed = Math.floor(Math.abs(currentScroll * 15));
        const randomFont = fonts[seed % fonts.length];
        const randomWeight = fontWeights[seed % fontWeights.length];
        const randomStyle = fontStyles[seed % fontStyles.length];
    ...
    return (
        <div ref={containerRef} className={`posters-container ${className}`} {...props}>
            ...
            {/* Center Fixed Kinetic Typography Layer */}
